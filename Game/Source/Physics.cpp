...
bool Physics::PreUpdate()
{
	...
	// we have to manually search for collisions and "call" the equivalent to the
	// ModulePhysics::BeginContact() ourselves...
	for (b2Contact *c = world->GetContactList(); c; c = c->GetNext())
	{
		...
		{
			// If so, we call the OnCollision listener function (only of the sensor), passing as
			// inputs our custom PhysBody classes
			auto *pb1 = (PhysBody *)c->GetFixtureA()->GetBody()->GetUserData();
			auto *pb2 = (PhysBody *)c->GetFixtureB()->GetBody()->GetUserData();

			if (pb1 && pb2 && pb1->listener) pb1->listener->OnCollision(pb1, pb2);
		}
	}
...
bool Physics::PostUpdate()
{
	...
	for (b2Body *b = world->GetBodyList(); b && (!selected || (selected && debugWhileSelected));
	     b = b->GetNext())
	{
			...
			switch (f->GetType())
			{
				...
				case b2Shape::Type::e_circle:
				{
					...
					app->render->DrawCircle(
						...
						METERS_TO_PIXELS(circleShape->m_radius),
						255,
						255,
						255
					);
...
{
	// Call the OnCollision listener function to bodies A and B, passing as inputs our custom
	// PhysBody classes
	...
	if (pBodyB && pBodyB->listener) pBodyB->listener->OnCollision(pBodyB, pBodyA);
}

//--------------- Create Shapes and Joints

PhysBody *Physics::CreateRectangle(
	int x,
	int y,
	int width,
	int height,
...
PhysBody *Physics::CreateCircle(
	int x, int y, int radius, BodyType type, float rest, uint16 cat, uint16 mask
)
{
...
PhysBody *Physics::CreatePolygon(
	int x,
	int y,
	const int *const points,
	int size,
	BodyType type,
	float rest,
	uint16 cat,
...
PhysBody *Physics::CreateRectangleSensor(
	int x, int y, int width, int height, BodyType type, uint16 cat, uint16 mask
)
{
	// Create BODY at position x,y
	b2BodyDef body;
...
b2RevoluteJoint *Physics::CreateRevoluteJoint(
	...
	iPoint anchorOffset,
	iPoint bodyOffset,
	std::vector<RevoluteJointSingleProperty> properties
)
{
...
b2PrismaticJoint *Physics::CreatePrismaticJoint(
	PhysBody *anchor,
	PhysBody *body,
	iPoint anchorOffset,
	iPoint bodyOffset,
...
b2MouseJoint *Physics::CreateMouseJoint(
	PhysBody *origin,
	PhysBody *target,
	b2Vec2 position,
	float dampingRatio,
	...
	switch (app->input->GetMouseButtonDown(SDL_BUTTON_LEFT))
	{
		...
		case KeyState::KEY_REPEAT:
		{
			...
			app->render->DrawLine(
				mouseX,
				mouseY,
				METERS_TO_PIXELS(selected->GetPosition().x),
				METERS_TO_PIXELS(selected->GetPosition().y),
				0,
				255,
				255,
				255
...
{
	if (f->TestPoint(IPointToWorldVec(app->input->GetMousePosition()))) return true;
	return false;
}
