...
InteractiveParts::InteractiveParts(pugi::xml_node const &itemNode = pugi::xml_node())
	: Entity(itemNode)
{}
...
bool InteractiveParts::Start()
{
	...
	if (parameters.attribute("hasfx"))
	{
		std::string audioFile =
			fxLevelPath + name + "." + parameters.attribute("hasfx").as_string();
	...
	if (parameters.attribute("function"))
		pBody->sensorFunction = (SensorFunction)parameters.attribute("function").as_int();
	else
		pBody->sensorFunction = SensorFunction::UNKNOWN;
	...
	switch (texture.type)
	{
		...
		case RenderModes::ANIMATION:
		{
			if (std::string(parameters.name()) != "anim_billboard")
			{
				...
				{
					const auto mainPos =
						app->physics->WorldVecToIPoint(flipperJoint->joint->GetAnchorB());
					const auto anchorPos =
						app->physics->WorldVecToIPoint(flipperJoint->joint->GetAnchorA());
					app->render->DrawTexture(
						...
						nullptr,
						1.0f,
						flipperJoint->joint->GetJointAngle(),
						flipperJoint->joint->GetAnchorA().x,
						flipperJoint->joint->GetAnchorA().y
					);
				}
				else
				{
					app->render->DrawTexture(
						texture.anim->GetCurrentFrame(), position.x, position.y
					);
				}
			}
			else
...
{
	// EntityType::ANIM are just animations of board, they don't have
	// collisions.
...
bool InteractiveParts::CreateCollidersBasedOnShape(const pugi::xml_node &colliderNode)
{
	...
	{
		LOG("Can't read xml attributes on "
		    "InteractiveParts::CreateCollidersBasedOnShape\n");
		LOG("ColliderShape is %s || BodyTypeChar is %s\n",
		    bodyTypeChar ? "true" : "false",
		    colliderShape.empty() ? "false" : "true");
		return false;
	}
	...
	{
		LOG("Attribute shape of %s not recognized in "
		    "InteractiveParts::CreateCollidersBasedOnShape",
		    std::string(colliderNode.name()));
		return false;
	}
...
PhysBody *InteractiveParts::CreateChainColliders(
	const std::string &xyStr, BodyType bodyT, std::string const &shape
)
{
	...
	{
		if (name == "bridge")
			border = app->physics->CreateChain(
				0,
				0,
				points.data(),
				std::distance(xyStrBegin, xyStrEnd),
				bodyT,
				0.0f,
				(uint)Layers::TOP,
				(uint)Layers::BALL
			...
			border = app->physics->CreateChain(
				0, 0, points.data(), std::distance(xyStrBegin, xyStrEnd), bodyT
			);
	}
	else
	{
		...
		if (name == "flipper")
			border = app->physics->CreatePolygon(
				posX,
				posY,
				points.data(),
				std::distance(xyStrBegin, xyStrEnd),
			...
			border = app->physics->CreatePolygon(
				posX, posY, points.data(), std::distance(xyStrBegin, xyStrEnd), bodyT
			);
	}
...
{
	if ((name != "flipper" || this->flipperJoint)
	    && (name != "launcher" || this->launcherJoint))
		return false;
	...
	for (pugi::xml_attribute attr : flipperNode.attributes())
	{
		...
		{
			if (name == "flipper")
				flipperHelper.motorSpeed = attr.as_float();
			else
				launcherHelper.motorSpeed = attr.as_float();
		}
		...
		switch (propertyToAdd.type)
		{
			...
			case RevoluteJoinTypes::UNKNOWN:
				LOG("Something went wrong in InteractiveParts doing the "
				    "revolute joint");
				break;
		}
	...
	else
	{
		...
		launcherHelper.joint = app->physics->CreatePrismaticJoint(
			launcherHelper.anchor, this->pBody, offset, { 0, 0 }, revoluteProperties
		);
	}
	...
	{
		this->launcherJoint = std::make_unique<LauncherInfo>(launcherHelper);
	}
	return true;
}
...
void InteractiveParts::AddTexturesAndAnimationFrames()
{
	...
	static const std::regex r(R"(([A-Za-z]+(?:_[A-Za-z]*)*)_(?:(image|static|anim)([\d]*)).png)"
	); // www.regexr.com/72ogq
	...
	while (n--)
	{
		...
		{
			if (foundOne)
				return; // As they are alphasorted, once we found one but the
				        // name isn't the same
			else
		...
		if (texture.type == RenderModes::UNKNOWN)
		{
			...
			if (texture.type == RenderModes::ANIMATION)
			{
				...
				auto animStyle =
					static_cast<AnimIteration>(parameters.attribute("animstyle").as_int());
				texture.anim->SetAnimStyle(animStyle);
				if (animStyle == AnimIteration::LOOP_FORWARD_BACKWARD
		...
		std::string match0 =
			interactiveFolder
			+ std::string(m[0]); // example: /Assets/Maps/ + triangle_left_anim001.png
		...
		switch (texture.type)
		{
			case RenderModes::ANIMATION:
				...
				if (match2 == "anim")
					texture.anim->AddSingleFrame(match0.c_str());
				else
					texture.anim->AddStaticImage(match0.c_str());
				break;
