...
struct RevoluteJointSingleProperty
{
	...
	RevoluteJointSingleProperty::RevoluteJointSingleProperty(
		const RevoluteJointSingleProperty &r
	)
		: type(r.type)
	{
		switch (type)
...
// Small class to return to other modules to track position and rotation of
// physics bodies
class PhysBody
{
public:
...
public:
	...
	PhysBody *CreateRectangle(
		...
		float rest = 1.0f,
		uint16 cat = (uint16)Layers::BOARD,
		uint16 mask = (uint16)Layers::BALL
	);
	...
	PhysBody *CreateRectangleSensor(
		int x,
		int y,
		int width,
		int height,
		BodyType type,
		uint16 cat = (uint16)Layers::SENSOR,
		uint16 mask = (uint16)Layers::BALL
	);
	PhysBody *CreateChain(
		int x,
		int y,
		const int *const points,
		int size,
